...
//  FitBuddy
//
//  Interest view with three tabs: Interested In Me, My Interests, Workout Partners
//
...
struct LikesView: View {
    ...
    private let tabs = ["Interested In Me", "My Interests", "Workout Partners"]
        ...
        Group {
            if viewModel.usersWhoLikedMe.isEmpty {
                emptyStateView(
                    ...
                    message: "When someone wants to train with you, they'll appear here. Keep browsing!"
                )
            } else if isPremium {
    ...
    private var premiumUnlockCard: some View {
        VStack(spacing: 20) {
            ...
            VStack(spacing: 8) {
                Text("\(viewModel.usersWhoLikedMe.count) people want to train with you!")
                    .font(.title2)
                    .fontWeight(.bold)

                Text("Upgrade to Premium to see who they are and partner up instantly")
                    .font(.subheadline)
                    .foregroundColor(.secondary)
    ...
    private var premiumFeaturesPreview: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(spacing: 12) {
                premiumFeatureRow(icon: "eye.fill", title: "See Who's Interested", description: "Partner up instantly with people who want to train with you", color: .orange)
                premiumFeatureRow(icon: "infinity", title: "Unlimited Interest", description: "No daily limits, connect with as many partners as you want", color: .green)
                premiumFeatureRow(icon: "bolt.fill", title: "Profile Boost", description: "Get 10x more views from potential workout partners", color: .blue)
            }
        }
        .padding(20)
        .background(
        ...
        Group {
            if viewModel.usersILiked.isEmpty {
                emptyStateView(
                    ...
                    message: "Start browsing on the Find Partners page to show interest!"
                )
            } else {
    ...
    private var mutualLikesTab: some View {
        Group {
            if viewModel.mutualLikes.isEmpty {
                emptyStateView(
                    icon: "person.2.fill",
                    title: "No Workout Partners Yet",
                    message: "When you and someone else both want to train together, you'll see them here!"
                )
            } else {

...
//  FitBuddy
//
//  Discovery filter preferences for finding workout partners
//
...
class DiscoveryFilters: ObservableObject {
    ...
    // Fitness Filters
    @Published var fitnessGoals: Set<String> = []
    @Published var fitnessLevels: Set<String> = []
    @Published var workoutTypes: Set<String> = []
    @Published var preferredWorkoutTimes: Set<String> = []
    @Published var workoutFrequencies: Set<String> = []
    ...
    func resetFilters() {
        ...
        fitnessGoals.removeAll()
        fitnessLevels.removeAll()
        workoutTypes.removeAll()
        preferredWorkoutTimes.removeAll()
        workoutFrequencies.removeAll()
    ...
    var hasActiveFilters: Bool {
        return minAge > 18 || maxAge < 65 || showVerifiedOnly || !selectedInterests.isEmpty ||
               !fitnessGoals.isEmpty || !fitnessLevels.isEmpty ||
               !workoutTypes.isEmpty || !preferredWorkoutTimes.isEmpty ||
               !workoutFrequencies.isEmpty || !dietPreferences.isEmpty ||
               !educationLevels.isEmpty || minHeight != nil || maxHeight != nil || !religions.isEmpty
    }
}
